...
// All authenticated users (Admin=1, Customer=2, Worker=3) can access tasks
app.use(roleCheck([1, 2, 3]));
...
// 1. Get All Tasks
app.get("/SortBy/Priority", async (req, res) => {
  try {
    const tasks = await Task.aggregate([
      ...
      { $sort: { priorityValue: -1 } }, // Sort by priority (descending), then by dueDate (ascending)
    ]);
...
// 1. Get All Tasks
app.get("/SortBy/DueDate", async (req, res) => {
  try {
...
// 1. Get All Tasks
app.get("/SortBy/Title", async (req, res) => {
  try {
